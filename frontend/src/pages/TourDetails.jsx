...
const TourDetails = () => {
  const {id} = useParams()// Extracts the tour ID from the URL
  const reviewsMsgRef = useRef('')
  const [tourRating, setTourRating,  ] = useState(null)
  const {user} = useContext(AuthContext)// Gets the current user from the authentication context
        ...
        <Container>
          ...
          <Col lg='8'>
           ...
           <div className='d-flex align-items-center justify-content-between'>
            ...
            <p>{
              new Date (review.createdAt).toLocaleDateString("en-US", options)}</p>
              </div>
